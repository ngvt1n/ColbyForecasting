...
```{r write_model_input}
write_model_input(variables, scientificname = SPECIES, version = "v2")
```
...
model_fits = workflowset_selectomatic(wflow, model_input_split,
                                  filename = "Temora_longicornis-v2-model_fits",
...
```{r load_workflow}
model_fits = read_model_fit(filename = "Temora_longicornis-v2-model_fits")
model_fits
```
