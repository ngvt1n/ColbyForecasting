...
cfg = list(
  ...
  method_background = "random",
  number_background = "observations per month",
  thinning = "false",
...
monthly_counts = count(st_drop_geometry(thinned_obs), month) # counting is faster without spatial baggage
...
obsbkg <- sapply(month.abb,
                 function(mon) {
                   sample_background(
                     ...
                     n = 1.5 * monthly_counts$n[mon |> match(month.abb)]) |>
...
model_fit_metrics(model_fits) |> 
  print() |> 
  capture.output() |> sapply(function(x) { paste("# ", x)}) |> write_lines(file = sprintf("make_all_Tlongicornis-%s.R", VERSION), append = TRUE)
...
# notes 
# both models: metrics really bad: roc_auc barely above 0.5 
# warnings in not being to generate enough background points 
# glm: sbttm 
# maxent: month, sbttm, sst
# super viable for tlongicornis, everywhere except along the continental shelf edge

#  # A tibble: 4 × 5
#    wflow_id       accuracy boyce_cont roc_auc tss_max
#    <chr>             <dbl>      <dbl>   <dbl>   <dbl>
#  1 default_glm       0.554      0.657   0.615   0.253
#  2 default_rf        0.485      0.857   0.629   0.227
#  3 default_btree     0.488      0.348   0.615   0.190
#  4 default_maxent    0.597      0.720   0.637   0.244
