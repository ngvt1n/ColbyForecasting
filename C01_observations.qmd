...
As always, we start by running our setup function. Start RStudio/R, and reload your project with the menu `File > Recent Projects`. Then source `setup.R`.  We'll also assign a new variable with our species name - we do that so it's easy to substitute in another species if needed.  We make it ALL CAPS so that it reminds us that it is more like a constant than a variable.

```{r source_setup, warning = FALSE}
source("setup.R")
SPECIES = "Meganyctiphanes norvegica"
```
